...
function BasicYearView(element, calendar, viewName) {
	...
	function updateCells(firstTime) {
		...
		subTables.each(function(i, _sub){
			...
			$(_sub).find('td').each(function(ii, _cell) {
				...
				if(!firstTime) {
					var class_array = parent.attr('class').split(' ')
					$.each(class_array, function(ia,id) {
						if(id.indexOf('fc-day')!==-1)  { //if the class name have already fc-day
							parent.removeClass(id); 
						}
						parent.removeClass("ui-state-disabled");
					});
				}
				parent.addClass(tm+"-widget-content fc-day" + dayStr);
	...
	function dayClick(ev) {
		if (opt('selectable')) { // if selectable, SelectionManager will worry about dayClick
			...
			var da = date.split("-");
			date = new Date(parseInt(da[0],10), parseInt(da[1],10)-1, parseInt(da[2],10), 0, 0, 0);
...
{	 
	...
	*/
	...
	eventEditDialog : function(event, jsEvent, view  )
	{
		...
		$('#className',dialogContent).val(event.className);
		...
		buttons[$.jqschedule.locales.bSave] = function() {
			...
			event.className = $('#className',dialogContent).val();
		...
		$(".ui-dialog").css({'font-size':'12px','z-index':1000004});
	...
	eventAddDialog : function(date, allDay, jsEvent, view)
	{
		...
		buttons[$.jqschedule.locales.bAdd] = function() {
			...
			event.className = $('#className',dialogContent).val();
			event.access = $('#access',dialogContent).val();
			if( $.trim(event.className) == '' || $.trim(event.access) ==='') { 
				alert("Category or Access are requiered!");
				return;
			}
	...
	changeCalendarUser : function()
	{
		...
		$(".ui-dialog, .ui-dialog-buttonpane").css({'font-size':'12px','z-index':1000004});
		return false;
	},
	/**
	...
	buildLocalSearch : function ()
	{
		...
		$('label', $.jqschedule.eventid).each(function(i){ 
			...
			sa.label = $(this).html();
			if(sa.name == 'className' || sa.name == 'access') {
				sa.inputtype ='select';
				so = {};
	...
	processScript :function() 
	{
		...
		$('#b_search_cal').toggleClick(jQuery.jqschedule.showSearch,jQuery.jqschedule.hideSearch);
